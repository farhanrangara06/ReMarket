...
import { getSentRequests, cancelRequest } from '../services/requestService';
...
const SentRequests = () => {
  ...
  const fetchRequests = async (page = 1) => {
    ...
    try {
      ...
      const { data, pagination: pag } = await getSentRequests(params);
      setRequests(data);
      setPagination(pag);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">My Purchase Requests</h1>
          <p className="text-gray-600 text-sm">Requests you&apos;ve sent to sellers</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Browse products and send a purchase request to start buying."
        />
      ) : (
        <>
